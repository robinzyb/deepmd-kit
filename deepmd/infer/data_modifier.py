...
from deepmd.infer.deep_dipole import DeepDipole, DeepFiniteFieldDipole
...
class FiniteFieldDipoleChargeModifier(DeepFiniteFieldDipole):
    """
    ...
    def __init__(self, 
        ...
        """
        # the dipole model is loaded with prefix 'dipole_charge'
        self.modifier_prefix = 'finitefielddipole_charge'
        # init dipole model
        DeepFiniteFieldDipole.__init__(self, 
                            model_name, 
                            load_prefix = self.modifier_prefix, 
                            default_tf_graph = True)
        self.model_name = model_name
    ...
    def eval(self, 
        ...
        # add wfcc
        all_coord, all_charge, dipole = self._extend_system(coord, box, atype, charge, field)
        ...
        if eval_fv:
            ...
            corr_av = []
            for ii in range(0,nframes,batch_size):
                f, v, av = self._eval_fv(coord[ii:ii+batch_size], box[ii:ii+batch_size], field[ii:ii+batch_size], atype, ext_f[ii:ii+batch_size])
                corr_f.append(f)
                corr_v.append(v)
                corr_av.append(av)
            corr_f = np.concatenate(corr_f, axis = 0)
    ...
    def _eval_fv(self, coords, cells, fields, atom_types, ext_f) :
        # reshape the inputs 
        cells = np.reshape(cells, [-1, 9])
        nframes = cells.shape[0]
        coords = np.reshape(coords, [nframes, -1])
        ...
        feed_dict_test[self.t_coord ] = coords.reshape([-1])
        feed_dict_test[self.t_box   ] = cells.reshape([-1])
        feed_dict_test[self.t_field ] = fields.reshape([-1])
        feed_dict_test[self.t_mesh  ] = default_mesh.reshape([-1])
        feed_dict_test[self.t_ef    ] = ext_f.reshape([-1])
        # print(run_sess(self.sess, tf.shape(self.t_tensor), feed_dict = feed_dict_test))
        fout, vout, avout \
    ...
    def _extend_system(self, coord, box, atype, charge, field):
        natoms = coord.shape[1] // 3
        nframes = coord.shape[0]
        # sel atoms and setup ref coord
        sel_idx_map = select_idx_map(atype, self.sel_type)
        ...
        all_dipole = []
        for ii in range(0,nframes,batch_size):
            dipole = DeepFiniteFieldDipole.eval(self,
                                     coord[ii:ii+batch_size],
                                     box[ii:ii+batch_size],
                                     atype,
                                     field[ii:ii+batch_size]
                                     )
            all_dipole.append(dipole)
        dipole = np.concatenate(all_dipole, axis = 0)
        assert(dipole.shape[0] == nframes)
    ...
    def modify_data(self, 
        ...
        atype = data['type'][:get_nframes,:]
        atype = atype[0]
        field = data['field'][:get_nframes,:]
        nframes = coord.shape[0]


        tot_e, tot_f, tot_v = self.eval(coord, box, atype, field)
